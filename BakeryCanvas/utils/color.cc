...
template <typename parsed_t> static bool parse_integer(const char** pStr, parsed_t* pParsed) {
    parsed_t& c = *pParsed;
    const char*& str = *pStr;
    int8_t sign = 1;
...
template <typename parsed_t> static bool parse_css_number(const char** pStr, parsed_t* pParsed) {
...
template <typename T> static T clip(T value, T minValue, T maxValue) {
    if (value > maxValue)
        value = maxValue;
    if (value < minValue)
        value = minValue;
...
template <typename T> static T wrap_float(T value, T limit) {
...
static bool parse_degrees(const char** pStr, float* pDegrees) {
    float degrees;
...
static bool parse_clipped_percentage(const char** pStr, float* pFraction) {
    float percentage;
...
#define ALPHA(NAME)                              \
    ...
    do {                                         \
    } while (0)  // require trailing semicolon

/*
 * Named colors.
 */
static const std::map<std::string, uint32_t> named_colors = { { "transparent", 0xFFFFFF00 },
...
void rgba_to_string(rgba_t rgba, char* buf, size_t len) {
    if (1 == rgba.a) {
        snprintf(buf, len, "#%.2x%.2x%.2x", static_cast<int>(round(rgba.r * 255)), static_cast<int>(round(rgba.g * 255)), static_cast<int>(round(rgba.b * 255)));
    } else {
        snprintf(buf, len, "rgba(%d, %d, %d, %.2f)", static_cast<int>(round(rgba.r * 255)), static_cast<int>(round(rgba.g * 255)), static_cast<int>(round(rgba.b * 255)), rgba.a);
    }
}
...
static inline int32_t rgba_from_rgba(uint8_t r, uint8_t g, uint8_t b, uint8_t a) {
...
static inline int32_t rgba_from_hsla(float h_deg, float s, float l, float a) {
    uint8_t r, g, b;
...
static inline int32_t rgba_from_hsl(float h_deg, float s, float l) {
...
static int32_t rgba_from_hex8_string(const char* str) {
    return rgba_from_rgba((h(str[0]) << 4) + h(str[1]), (h(str[2]) << 4) + h(str[3]), (h(str[4]) << 4) + h(str[5]), (h(str[6]) << 4) + h(str[7]));
}
...
static int32_t rgba_from_hex6_string(const char* str) {
...
static int32_t rgba_from_hex4_string(const char* str) {
    return rgba_from_rgba((h(str[0]) << 4) + h(str[0]), (h(str[1]) << 4) + h(str[1]), (h(str[2]) << 4) + h(str[2]), (h(str[3]) << 4) + h(str[3]));
}
...
static int32_t rgba_from_hsla_string(const char* str, short* ok) {
...
static int32_t rgba_from_hsl_string(const char* str, short* ok) {
...
static int32_t rgba_from_hex_string(const char* str, short* ok) {
    ...
    *ok = 1;
    switch (len) {
        ...
        case 4:
            return rgba_from_hex4_string(str);
        case 3:
...
void rgba_inspect(int32_t rgba) {
